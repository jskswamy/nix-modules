let
  ...
  lib = pkgs.lib;
  mk = extra: (hm.lib.homeManagerConfiguration {
    inherit pkgs;
    modules = [{home.username="t"; home.homeDirectory="/home/t"; home.stateVersion="24.05";} extra];
  }).config;
  T = nm.homeManagerModules.tools;
  pkgNames = c: map (p: p.name or "?") c.home.packages;
in {
  # A. enabling a tool installs it
  installsTool = let c = mk {imports=[T.nvim];}; in {
    neovimInstalled = builtins.any (n: lib.hasPrefix "neovim" n) (pkgNames c);
  };
  # B. override which package
  overridePackage = let c = mk {imports=[T.tig]; tools.tig.package = pkgs.hello;}; in {
  ...
  configOnly = let c = mk {imports=[T.tig]; tools.tig.package = null;}; in {
  ...
  overrideSetting = let c = mk {imports=[T.alacritty]; programs.alacritty.settings.window.opacity = 0.8;}; in {
    opacity = c.programs.alacritty.settings.window.opacity;
  };
  # E. replace a whole dotfile the module ships
  replaceFile = let c = mk {
    imports=[T.tig];
  ...
  theme = let c = mk {imports=[T.alacritty]; theme.variant = "catppuccin";}; in {
  ...
  overlayNotApplied = let c = mk {imports=[T.hunk];}; in {
